...
The thinking, memorialized. The scaffold you're reading (philosophy, tiers, patterns, Loea design) ships with this milestone; what remains is grounding it in measurement:
...
The keystone tool, built to its own rules ([design](design/loea/overview.md)):

- **Loea v0 implementation.** The full journey: setup interview → tier determination → budget-fit skill design → transparency pass → manifest + scaffold generation.
...
- **Permission grants file.** `~/.kamaaina/grants.yaml` read/write, presentation generation, and escalation-on-diff per the [permission model](design/loea/permission-model.md).
...
Loea proves itself on real frontier-era skills:

- **Refactor teach-me.** The 953-line monolith becomes a tier-declared tool: routed phases, externalized invariants, page-at-a-time generation, deterministic verification, offline-safe. The [context-budget case study](design/loea/context-budget.md#case-study-teach-me-blows-the-budget) is the spec of the problem.
